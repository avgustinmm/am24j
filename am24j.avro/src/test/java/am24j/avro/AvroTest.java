...
import am24j.bean.BeanTest.BeanStd;
import am24j.bean.BeanTest.Buildable;
import am24j.bean.BeanTest.BuildableNewBuilder;
import am24j.bean.BeanTest.BuildableReq;
import am24j.bean.BeanTest.Buildable.Builder;
...
public class AvroTest {
  ...
  private static void check(final Object bean) throws IOException {
    {
      final byte[] ba = Avro.write(bean, Encoding.Json);
      final Object decoded = Avro.read(ba, bean.getClass(), Encoding.Json);
      Assert.assertEquals(bean, decoded);
    }

    {
      final byte[] ba = Avro.write(bean, Encoding.Binary);
      final Object decoded = Avro.read(ba, bean.getClass(), Encoding.Binary);
      Assert.assertEquals(bean, decoded);
    }
  }
//  
