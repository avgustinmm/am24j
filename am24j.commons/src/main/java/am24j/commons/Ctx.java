 ...
 * <ul>
 *   <li>Property configuration, based on environment properties and system properties</li>
 *   <li>Resources resolved by path</li>
 *   <li>Logger creation by name (in the {@link Ctx#NAMESPACE_LOG})</li>
 *   <li>Files in read only / or read write directories. Read only directory may be assumed (used as) configuration directory
 *     while read / write directory may be used as temporary or persistent application context directory</li>
 *   <li>Call context - attach context to an execution. It is mapped to the current thread.</li>
 * </ul>
 *
 * Properties:<br>
 * <ul>
 *   <li>{@link #NAMESPACE_ENV} and {@link #NAMESPACE_SYS} are resolved as properties using but with default
 *     namesoaces ({@link #DEFAULT_NAMESPACE_ENV} and {@link #DEFAULT_NAMESPACE_SYS})</li>
 *   <li>Properties are static - once a property is resolved the application will always get the same property.
 *     I.e. once a property is resolved, changing correspondent system property won't change the value</li>
 *   <li>Property 'x.y.z' is resolved from (in that order, assuming {@link #NAMESPACE_ENV} is 'NS' and
 *     {@link #NAMESPACE_SYS} is 'ns'):
 *    <ol>
 *      <li>environment variable: NS_X_Y_Z ({@link #NAMESPACE_ENV}_[property to upper case with dots replaced by '_']</li>
 *      <li>system property: ns.x.y.z ({@link #NAMESPACE_SYS}.[property]</li>
 *    </ol>
 *   </li>
 * </ul>
 *
 * Resources are resolved by path in following order:
 * <ol>
 *   <li>from file system relatively to {@link Ctx#RO_DIR} (read only, resource or configuration directory)</li>
 *   <li>as a resource resolved by {@link Ctx}'s class loader.</li>
 * </ol>
 *
 *
 * @author avgustinmm
...
public class Ctx {
  ...
  public static final File RO_DIR = new File(prop("fs.roDir", "."));
  public static final File RW_DIR = new File(prop("fs.rwDir", "."));
  static {
    if (RW_DIR.isFile()) {
