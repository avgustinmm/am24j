...
* **RPC** - RPC over Vertx runtime. It supports two implementations - gRPC and Http with Avro encoding. It supports unary calls and server streaming 
...
* pack a whole application and its dependencies in single jar with dependencies being packed as sub-jars (no need to be unpacked). This allows easy observing of dependencies, versions and do  not mix them.
...
am24j provides a simple example for such application. Example is a jar, example-app.jar which pack its dependencies as sub-jars, with _Main-Class am24j.launch.Launcher_ which is instructed (with argunments, to start / launch application class _am24j.example.App_.<br/>
...
#### Starter
Injector provides a Starter utility that provides injector with some predefined resolvers and interceptors that, based on list of classes, instantiates them (in ordered manner) and (on Starter.close()) finally closed auto closables.<br/>
...
The example for Starter above shows how, for instance, a RPC server may be bootstrapped. For more in detail use of RPC you may take a look at RPC tests.
